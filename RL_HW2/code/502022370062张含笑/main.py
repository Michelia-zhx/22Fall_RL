...
def plot(record, args):
	...
	ax.set_title('Performance in Env Mode '+str(args.env_mode))
	# ax1 = ax.twinx()
	...
	fig.savefig('performance_env'+str(args.env_mode)+'.png')
...
def main():
	...
	# environment initial
	envs = Make_Env(env_mode=args.env_mode)
	action_shape = envs.action_shape
	observation_shape = envs.state_shape
	print(action_shape, observation_shape)

	discrete_observation_shape = (envs.grid_num, envs.grid_num)
	# agent initial
	# you should finish your agent with QAgent
	agent = myQAgent(action_shape, discrete_observation_shape, args)
	# agent = QAgent()
	...
	for i in range(num_updates):
		...
		for step in range(args.num_steps):
			...
			epsilon = max(0, 0.8 - i / num_updates)
			...
			if args.save_img:
				# scipy.misc.toimage(info, cmin=0.0, cmax=1).save()
				Image.fromarray((info[0]*255).astype('uint8'), mode='L').convert('RGB').save('imgs/example.jpeg')
